...
const PhoneMockup = ({ size = "lg" }: { size?: "lg" | "sm" }) => {
  ...
  return (
    <div className={isSmall ? "w-32" : "w-64"}>
      <div
        className={`bg-[#1a1f24] rounded-[1.75rem] shadow-2xl ${isSmall ? "p-1.5" : "p-2.5"}`}
      >
        <div className="bg-white rounded-[1.4rem] overflow-hidden">
          <div
            className={`flex items-center justify-between ${isSmall ? "px-2.5 pt-2.5 pb-1" : "px-4 pt-4 pb-2"}`}
          >
            <span
              className={`font-bold text-primary ${isSmall ? "text-[9px]" : "text-sm"}`}
            >
              Testraca
            </span>
            <Menu
              className={
                isSmall ? "w-2.5 h-2.5 text-gray-400" : "w-4 h-4 text-gray-400"
              }
            />
          </div>
          <div
            className={
              isSmall ? "px-2.5 pb-2.5 space-y-1.5" : "px-4 pb-4 space-y-3"
            }
          >
            <div
              className={`bg-white border border-border rounded-lg shadow-sm ${isSmall ? "p-1.5" : "p-3"}`}
            >
              <p
                className={
                  isSmall
              ...
              <p
                className={`font-bold text-foreground ${isSmall ? "text-[8px]" : "text-sm"}`}
              >
                Lab Test #4920
              </p>
              <div
                className={`flex items-center gap-1 text-muted-foreground mt-0.5 ${isSmall ? "text-[6px]" : "text-[10px]"}`}
              >
            ...
            <div className="grid grid-cols-2 gap-1.5">
              <div
                className={`bg-teal-surface rounded-lg flex flex-col items-center gap-0.5 ${isSmall ? "p-1" : "p-2"}`}
              >
                <Microscope
                  className={
                    isSmall
                ...
                />
                <span
                  className={`font-semibold text-primary ${isSmall ? "text-[6px]" : "text-[10px]"}`}
                >
                  Tests
                </span>
              </div>
              <div
                className={`bg-muted rounded-lg flex flex-col items-center gap-0.5 ${isSmall ? "p-1" : "p-2"}`}
              >
                <MessageSquare
                  className={
                    isSmall
                ...
                />
                <span
                  className={`font-semibold text-gray-600 ${isSmall ? "text-[6px]" : "text-[10px]"}`}
                >
                  Chat
                </span>
              </div>
            ...
            <div
              className={`rounded-lg bg-gradient-to-br from-primary to-primary/70 relative overflow-hidden ${isSmall ? "h-12" : "h-24"}`}
            >
              ...
              <div
                className={`absolute rounded-full bg-white/90 flex items-center justify-center ${isSmall ? "top-1 right-1 w-3 h-3" : "top-2 right-2 w-6 h-6"}`}
              >
                <User
                  className={
                    isSmall
                      ? "w-1.5 h-1.5 text-primary"
...
const finderFilled = (r: number, c: number) => {
  ...
  const [cr, cc] = corners.find(
    ([cr0, cc0]) => r >= cr0 && r < cr0 + 7 && c >= cc0 && c < cc0 + 7,
  )!;
  const lr = r - cr;
  const lc = c - cc;
  ...
  <>
    {checklist.map((item) => (
      <div key={item} className="flex items-center gap-3">
        ...
        <span className={itemClassName || "text-sm font-bold text-foreground"}>
          {item}
        </span>
      </div>
    ))}
...
const AppStoreButtons = ({ stacked }: { stacked?: boolean }) => (
  <div
    className={
      stacked ? "flex flex-col gap-3" : "flex flex-col sm:flex-row gap-4"
    }
  >
    ...
    <div className="min-h-screen bg-canvas flex flex-col">
      ...
      <main className="sm:hidden flex-1 px-4 pt-8 pb-16">
        ...
        <p className="text-sm text-muted-foreground leading-relaxed mb-6">
          The full web version is coming soon. For now, manage your entire
          health journey directly from your pocket.
        </p>
        ...
        <div className="container mx-auto max-w-6xl grid grid-cols-2 gap-16 items-center py-16">
          ...
          <div>
            ...
            <p className="text-base text-muted-foreground leading-relaxed max-w-lg mb-8">
              Unlock the complete Testraca experience. Manage your entire health
      ...
      <footer className="border-t border-border py-6 px-4 md:px-8">
        <div className="container mx-auto max-w-6xl flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-muted-foreground">
          <p>
            &copy; {new Date().getFullYear()} Testraca Health Logistics. All
            rights reserved.
          </p>
          <div className="flex items-center gap-4">
            <a
